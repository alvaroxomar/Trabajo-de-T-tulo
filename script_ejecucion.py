if __name__ == "__main__":
    ...
    def guardar_datos(distancias, campo_electrico, densidad_corriente, ruta=".", nombre_csv="datos.csv", nombre_excel="datos.xlsx", guarda=False):
        """
        Guarda las listas de distancias, campo eléctrico y densidad de corriente
        en archivos CSV y Excel con formato de 3 columnas y encabezados estandarizados.
        ...
        if guarda:
            ...
            # Crear un DataFrame con encabezados estandarizados
            data = {
            ...
            if os.path.exists(ruta_csv):
                ruta_csv = os.path.join(ruta, f"new_{nombre_csv}")
                print(f"Archivo CSV existente. Guardando como '{ruta_csv}'.")

            if os.path.exists(ruta_excel):
                ruta_excel = os.path.join(ruta, f"new_{nombre_excel}")
                print(f"Archivo Excel existente. Guardando como '{ruta_excel}'.")

            # Guardar como CSV
            df.to_csv(ruta_csv, index=False)
            print(f"Datos guardados como '{ruta_csv}'.")

            # Guardar como Excel
            df.to_excel(ruta_excel, index=False, sheet_name="Campo y Corriente")
            print(f"Datos guardados como '{ruta_excel}'.")

    def guarda_en_carpeta(Vol, ycoor, nx, ny, x, Ei, Ji, guarda=False):
        """
        ...
        Args:
            ...
            guarda (bool): Indicador para guardar o no los archivos.
        """
        ruta_destino = r"C:\\Users\\HITES\\Desktop\\la uwu\\14vo semestre\\Trabajo de título\\programa resultados"
    ...
    Pres = float(params["Presión (Pa)"]) # Pa
    ...
    def show_plot(graf):
        ...
        plt.show()
    guarda_en_carpeta(Vol, y_coor, nodosx, nodosy, x, Ei, Ji, guarda=guarda)
    show_plot(gra)

...
Sx = 20 # (m) media longitud del plano de tierra 
Sy = 27 # (m) altura del área de estudio respecto de tierra
...
E = np.zeros((150,150))
Exx = np.zeros((150,150))
Eyy = np.zeros((150,150))
#Eu = np.zeros((200,200))
...
Jp = 2*10**(-3) # (A/m^2) Densidad de corriente iónica promedio sobre el plano de tierra (Se debe adivinar este valor)
# Condiciones de borde
rho_i = (Sx*Jp)*10**(-3)/(np.pi*mov*(100*R)*m*(30*delta + 9*np.sqrt(delta/(100*R)))) # radio está en cm
rho = np.zeros((150,150))*10**(-8)
...
Edef = np.zeros((150,150))
Edefx = np.zeros((150,150))
Edefy = np.zeros((150,150))
J = np.zeros((150,150))
Ei = np.zeros(150)
Ji = np.zeros(150)
Jpp = []

...
def update_rho_vectorized(rho, Ex, Ey, dx, dy, epsilon0, wx, wy, rho_bound):
    ...
    rho[1:-1, 1:-1] = np.where(G >= 0, np.sqrt(G), np.sqrt(np.abs(G)))
    ...
    Gei = -epsilon0 * (Ewx[0, 0] * (rho[0, 1] - rho[0, 0]) / dx + Ewy[0, 0] * (rho[1, 0] - rho[0, 0]) / dy)
    Ged = -epsilon0 * (Ewx[0, -1] * (rho[0, -1] - rho[0, -2]) / dx + Ewy[0, -1] * (rho[1, -1] - rho[0, -1]) / dy)
    #rho[1:-1, 0] = np.where(Gi >= 0, np.sqrt(Gi), np.sqrt(np.abs(Gi)))
    rho[1:-1, 0] = np.where(Gi >= 0, np.sqrt(Gi), 0)
    #rho[1:-1, -1] = np.where(Gd >= 0, np.sqrt(Gd), np.sqrt(np.abs(Gd)))
    rho[1:-1, -1] = np.where(Gd >= 0, np.sqrt(Gd), 0)
    #rho[0, 1:-1] = np.where(Gs >= 0, np.sqrt(Gs), np.sqrt(np.abs(Gs)))
    rho[0, 1:-1] = np.where(Gs >= 0, np.sqrt(Gs), 0)
    rho[0, 0] = np.where(Gei >= 0, np.sqrt(Gei), np.sqrt(np.abs(Gei)))
    rho[0, -1] = np.where(Ged >= 0, np.sqrt(Ged), np.sqrt(np.abs(Ged)))
...
def algoritmo_rho_v(V, dx, dy, windx, windy,max_iter_rho):
    ...
    for iteration in range(max_iter_rho):
        ...
        condicion = np.all(diferencia_relativa < 0.1)

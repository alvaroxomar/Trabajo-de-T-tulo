if __name__ == "__main__":
    ...
    def algoritmo_rho(sag, V, Vp, Vn, Evvs, Vcos, Vos, rho_inip, rho_inin, rho_b, max_iter_rho, dx, dy, movp, movn, wx,wy, val_cond, pos, condi='si', copia='no',
        ...
        Vconn = [Vcos[1]]
        Vonn = [Vos[1]]
        Exp1, Eyp1,Emp1 = calcular_campo_electrico(Vp, dx, dy, posp, Evvp, Vcop, Vop, kapzov_borde=True, bundle=is_bundled)
        Exp1w = Exp1*movp + wx
        Eyp1w = Eyp1*movp + wy
        Exn2, Eyn2,Emp2 = calcular_campo_electrico(Vn, dx, dy, posn, Evvnn, Vconn, Vonn, kapzov_borde=True, bundle=is_bundled)
        '''
...
#print(f"malla X {X}")
    ...
    def grafJ1(num, ruta, mostrar=False, guarda=False):
        ...
        plt.plot(x[30:-30], Jtot[coordenada_med,30:-30]*(10**9), label=r"$|J|_{ave}$ ="f"{formatear_numero(Jave*(10**9))}"+r" $nA/m^2$")# perfil del módulo de ladensidad de corriente iónica total
    ...
    def grafE1(num, ruta, mostrar=False, guarda=False):
        plt.figure(num)
        plt.plot(x[30:-30], -Edefy[coordenada_med,30:-30]/1000, label=r"$E_{y,ave}$"f"= {formatear_numero(np.mean(np.abs(Edefy[coordenada_med,:]/1000)))} kV/m")
        plt.plot(x[30:-30], -Eyyini[coordenada_med,30:-30]/1000, label=r"$Ee_{y,ave}$"f"= {formatear_numero(np.mean(np.abs(Eyyini[coordenada_med,:]/1000)))} kV/m")
        plt.plot(x[30:-30], Edef[coordenada_med,30:-30]/1000,linestyle='-.', label=r"$|E|_{ave}$"f"= {formatear_numero(np.mean(np.abs(Edef[coordenada_med,:]/1000)))} kV/m")
        plt.plot(x[30:-30], Emi[coordenada_med,30:-30]/1000,linestyle='-.', label=r"$|Ee|_{ave}$"f"= {formatear_numero(np.mean(np.abs(Emi[coordenada_med,:]/1000)))} kV/m")

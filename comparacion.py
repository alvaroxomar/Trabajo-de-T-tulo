...
for idx, archivo in enumerate(archivos_excel):
    ...
    if idx==0:
        plt.title(f"Campo Eléctrico vs Posición, $E_{{on}}={np.round(Esup_der, 2)}$ kV/cm", fontsize=15)
    if "Campo y Corriente" in hojas:
        ...
        if {"Lateral Distance x[m]", "E_y [kV/m]"}.issubset(df.columns):
            ...
            Eav = np.mean(Eprom)
            plt.plot(distancia, campo_electrico, label=f"Wx={Wx} m/s, $\\alpha$={alpha} ,|E|= {np.round(Eav,2)} kV/m",
                     color=colores[idx], marker=marcadores[idx], linestyle='-', linewidth=1, markersize=5)
            ...
            ax1 = plt.gca()
            ax2 = ax1.twinx()
            ax2.scatter(x1, y1, color='purple', marker='o', label="Polo izquierdo")  # Agregar el punto
            ax2.scatter(x2, y2, color='purple', marker='o', label="Polo derecho")  # Agregar el punto
...
for idx, archivo in enumerate(archivos_excel):
    ...
    alpha = dic_ambientales.get("Rugosidad terreno", ["N/A"])[0] if dic_ambientales else "N/A"
    if idx==0:
        plt.title(f"Densidad de Corriente vs Posición, $E_{{on}}={np.round(Esup_der, 2)}$ kV/cm", fontsize=15)
    if "Campo y Corriente" in hojas:
        df = hojas["Campo y Corriente"]
        if {"Lateral Distance x[m]", "J [nA/m^2]"}.issubset(df.columns):
            ...
            plt.plot(distancia, corriente, label=f"Wx={Wx} m/s, $\\alpha$={alpha}, Jav= {formatear_numero(np.round(Jav,2))} nA/$m^{2}$",
                     color=colores[idx], marker=marcadores[idx], linestyle='-', linewidth=1, markersize=5)
